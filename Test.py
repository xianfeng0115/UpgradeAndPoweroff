...
class matchbug(tk.Tk):
    def __init__(self):
        ...
        self.scale = tk.Scale(self, label='下电时间调整', from_=43, to=45, orient=tk.HORIZONTAL, length=130, showvalue=0,
    ...
    def test(self):
        while True:
            ...
            os.popen('D:\pythonCode\MyTools\\UpgradeAndPoweroff\PowerOff/M2MdiagnosticPlatform.Power.exe')
    ...
    def checksuccess(self):
        while True:
            try:
                window2 = uiautomation.WindowControl(searchDepth=1, AutomationId='DownloadStatusForm')
                text=window2.TextControl(ClassName="TextBlock", AutomationId='tbxDownloadStatus').Name
            except LookupError:
                pass
            else:
                os.popen('D:\pythonCode\MyTools\\UpgradeAndPoweroff\PowerOff/M2MdiagnosticPlatform.Power.exe')
